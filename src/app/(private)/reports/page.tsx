...
function Reports({ search }: { search: string }) {
  const [allRows, setAllRows] = React.useState<DashboardReport[]>([]);
  ...
  const loadReports = React.useCallback(async () => {
    ...
    try {
      ...
      const data = (await res
        .json()
        .catch(() => ({}))) as DashboardReportsResponse;
      ...
      setAllRows(Array.isArray(data.reports) ? data.reports : []);
    } catch {
  ...
  const openDetails = React.useCallback((row: DashboardReport) => {
    setDetailsReport(row);
    setDetailsOpen(true);
  }, []);
  ...
  return (
    <div className="grid gap-4">
      ...
      <FeedbackDetailsModal
        open={detailsOpen}
        kind="report"
        value={detailsReport?.reason ?? null}
        message={detailsReport?.details ?? null}
        ...
        {rows.map((r) => (
          <Row key={r.id} columns="2.2fr 1.2fr 1fr 1.2fr">
            ...
            <div className="truncate text-black/70">
              {getUsername(r.reporterUsername ?? r.reporter)}
            </div>
      ...
      {loading ? (
        <Card className="p-6 text-sm text-black/70">Loading reports…</Card>
      ) : null}
