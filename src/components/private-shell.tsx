...
const NAV: Array<{ key: NavKey; label: string; icon: LucideIcon }> = [
  ...
  { key: 'listings', label: 'Listings', icon: PackageCheck },
...
function getActiveNavKey(pathname: string): NavKey {
  ...
  return NAV.some((n) => n.key === key) ? key : 'listings';
}

function PrivateShellInner({
  children,
}: Readonly<{
  ...
  const sectionLabel = React.useMemo(() => {
    const item = NAV.find((entry) => entry.key === nav);
  ...
  const logout = React.useCallback(async () => {
    ...
    try {
      ...
      await fetch(`${apiUrl}/api/dashboard/auth/logout`, {
        method: 'POST',
        credentials: 'include',
      });
    } catch {
  ...
  return (
    <div className="min-h-screen bg-white text-black">
      <div className="pointer-events-none fixed inset-0">
        <div className="absolute -top-40 left-1/2 h-130 w-130 -translate-x-1/2 rounded-full bg-black/5 blur-3xl" />
        <div className="absolute top-56 -left-30 h-105 w-105 rounded-full bg-black/5 blur-3xl" />
        <div className="absolute -bottom-45 -right-30 h-130 w-130 rounded-full bg-black/5 blur-3xl" />
      </div>
              ...
              {NAV.map((entry) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      if (!active) navigateTo(entry.key);
                    }}
